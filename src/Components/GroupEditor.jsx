    ...
    const handleSaveAttendance = async (data) => {
        try {
            ...
            if (data.grades && Object.keys(data.grades).length > 0) {
                ...
                if (scheduleItem.group_id) {
                    gradesUrl = `http://localhost:3001/api/schedules/${scheduleItem._id}/grades`;
                    gradesBody = { grades: data.grades };
                } else {
                    const studentId = Object.keys(data.grades)[0];
                    gradesUrl = `http://localhost:3001/api/schedules/${scheduleItem._id}/grade`;
            ...
            const updatedResponse = await fetch(`http://localhost:3001/api/schedules/${scheduleItem._id}`);
