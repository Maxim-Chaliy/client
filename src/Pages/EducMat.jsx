...
const EducMat = () => {
    ...
    const placeholderImage = `http://localhost:3001/uploads/to/placeholder.png`;

    const userRole = localStorage.getItem('role');

    const fetchMaterials = async () => {
        try {
            const response = await fetch(`http://localhost:3001/api/educmat`);
    ...
    const handleDeleteClick = async (materialId) => {
        try {
            const response = await fetch(`http://localhost:3001/api/educmat/${materialId}`, {
                method: 'DELETE',
            });
                                                ...
                                                <div className="img-block-material">
                                                    <img
                                                        src={material.image && material.image[0] ?
                                                            `http://localhost:3001/uploads/images/${material.image[0]}` :
                                                            placeholderImage}
                                                        alt={material.title}
                                ...
                                <div className="educmat-modal-img">
                                    <img
                                        src={selectedMaterial.image && selectedMaterial.image[0] ?
                                            `http://localhost:3001/uploads/images/${selectedMaterial.image[0]}` :
                                            placeholderImage}
                                            ...
                                            <ul className="educmat-modal-ul">
                                                {selectedMaterial.file.map((fileName, index) => (
                                                    <li className="educmat-modal-li" key={index}>
                                                        <a
                                                            href={`http://localhost:3001/uploads/materials/${fileName}`}
                                                            target="_blank"
